...
import fr.irit.tetralogie.swing.ControlWindow;
...
public class Sommet {
	...
	public int classe;
	public ControlWindow control = null;
	...
	public Font fnt1 = new Font("Time", Font.PLAIN, 9);
	 ...
	 *            ordonnee du sommet
	 */
	public Sommet(int x, int y, int z) {
		this.x = x;
		this.y = y;
		this.z = z;
		couleur = Color.red;
	 ...
	 *            etiquette du sommet
	 */
	public Sommet(int x, int y, int z, Color couleur, String nom,
			int nbvoisins, int seuil_voisin, int num) {
		this(x, y, z, couleur);
		this.nom = nom;
	 ...
	 *            etiquette du sommet
	 */
	public Sommet(int x, int y, int z, String nom, int nbvoisins,
			int seuil_voisin, int num, int modifx, int modify) {
		this(x, y, z);
		this.nom = nom;
		this.nbvoisins = nbvoisins;
		this.seuil_voisin = seuil_voisin;
	...
	public synchronized void afficher(Graphics g, boolean Etiquette,
		...
		if (this.getVisible()) {
			if (max != 0.0)
				cl = metrique / max;
			if (forme == 0) { // sommet sous forme circulaire avec nuances de
								// couleurs
				if (clustering) {
					g.setColor(new Color((Math.abs(couleurClasse.getRed())),
							(Math.abs(couleurClasse.getGreen())), (Math
									.abs(couleurClasse.getBlue()))));
