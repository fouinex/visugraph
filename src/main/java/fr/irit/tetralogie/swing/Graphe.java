...
import fr.irit.tetralogie.model.NotOrientedGraphException;
...
public class Graphe extends JPanel {
		...
		for (int i = 0; i < sommets.size(); i++) {
			...
			if (s.getVisible()) {
				if (typeMat == 0) {
					if (s.type == 1) {
						cl = (s.metrique) / (max1);
						cl = (1 + Math.pow(cl, 1 / 2) - Math.pow((1 - cl), 2)) / 2;
	...
	public synchronized void SetCouleursAretes(double intensite, int fond) {
		...
		for (int i = 0; i < aretes.size(); i++) {
			...
			Arete a = (Arete) aretes.elementAt(i);
			if (a.getVisible()) {
				metrique = a.getVal() / maxArete;

				/*
				 * on regle l'intensitï¿½ï¿½ d'affichage. C'est ici que se
				 * gï¿½ï¿½re la diffï¿½ï¿½rence d'intensitï¿½ï¿½ du slider
				 * intensite
				 */
				...
				if (fond == 0) {
					cl = (1.0 + Math.pow(metrique, 1 / intensite) - Math.pow(
							1 - metrique, intensite)) / 2.0;
					val1 = 230;
					val2 = 225;
				} else {
					cl = (1.0 + Math.pow(1 - metrique, intensite) - Math.pow(
							metrique, 1 / intensite)) / 2.0;
					val1 = 240;
					val2 = 200;
				}
