...
public class FishEyeMenu extends JMenu implements MenuListener {
	...
	public void menuSelected(MenuEvent e) {
		...
		fishEyeWindow.setLocation(location.x + 45, location.y
				+ getSize().height);
...
class FishEyeWindow extends JWindow implements MouseListener,
MouseMotionListener, KeyListener, ItemListener {
	...
	public FishEyeWindow(FishEyeMenu menu) {
		...
		/*
		 * if (up == null) { URL resource =
		 * this.getClass().getClassLoader().getResource("arrow-up.gif"); up =
		 * Toolkit.getDefaultToolkit().createImage(resource); resource =
		 * this.getClass().getClassLoader().getResource("arrow-down.gif"); down
	...
	/**
	 * Calculate the sizes of the window, the max and min font sizes, and the
	 * focus length. It uses the following the algorithm: Try to follow desired
	 * max font size and focus length by first computing the biggest minimum
	 * font size that will fit in the available space. If this is not satisfied
	 * for a minimum font size of 1, then first the focus length, and then the
	 * max font size is reduced until it fits.
	 */
	public void calculateSizes() {
		...
		for (i = 0; i < numItems; i++) {
			stringWidth = (float) fonts[maxFontSize].getStringBounds(
					items[i].label, frc).getWidth() + 5;
		...
		maxHeight = screenSize.height - location.y - 30; // Leave space for
		// Start Menu Bar
		...
		do {
			for (size = maxFontSize; size >= 1; size--) {
				...
				if (tmpHeight <= maxHeight) {
					...
					minFontSize = size;
					if (size == maxFontSize) {
						// If whole window at max size, then no compressed
						// spacing
			...
			if (!done) {
				// If not done, there wasn't enough room, so try a smaller focus
				// or max font size
				if (focusLength > 2) {
					focusLength -= 2;
				} else {
	 ...
	 * fisheye distortion using the class variables such as # of elements, max
	 * font size, spacing, and focus length, and the specified minimum font
	 * size.
	 */
	public int calculateWindowHeight(int minFontSize) {
		...
		if (minFontSize == maxFontSize) {
			...
			height = n * (maxFontSize + desiredSpace);
			return height;
		}
		...
		height = fl * (maxFontSize + desiredSpace);
		n -= fl;
	...
	public void ecrire(String nom) throws IOException {
		champ_req = champ_req + " / "
				+ items[selectionIndex].menuItem.getLabel();
	...
	private void init() {
		...
		desiredSpace = (int) (maxFontSize * desiredSpacing);
	...
	public void keyPressed(KeyEvent e) {
		...
		default:
			...
			if (Character.isLetter(c)) {
				...
				if (Character
						.toUpperCase(items[selectionIndex].label.charAt(0)) == c) {
					selectionIndex++;
					focusIndex++;
					repaint();
				}
				...
				if ((selectionIndex >= numItems)
						|| (Character.toUpperCase(items[selectionIndex].label
								.charAt(0)) != c)) {
	...
	public void mouseMoved(MouseEvent e) {
		...
		if (e.getX() < getSize().width / 2) {
			...
			if (!focusLock) {
				// Just entered focus lock mode, so update look up tables, and
				// calculate focus item position
		...
		if (focusLock) {
			...
			if (mouseY > flFocusPosition) {
				...
				// First, set the items between the focus and the cursor to full
				// size
				...
				int i;
				int y = flFocusPosition - flSizeLUT[focusIndex - 1]
						- flSpaceLUT[focusIndex - 1];
				selectionIndex = focusIndex;
	...
	public void mousePressed(MouseEvent e) {
		try {
			ecrire(items[selectionIndex].menuItem.getLabel());
	...
	public void paint(Graphics g) {
		...
		((Graphics2D) gb).setRenderingHint(
				RenderingHints.KEY_TEXT_ANTIALIASING,
				RenderingHints.VALUE_TEXT_ANTIALIAS_OFF);
		gb.setClip(clipBounds.x, clipBounds.y, clipBounds.width,
				clipBounds.height);
		...
		if (!focusLock) {
			for (i = 0; i < numItems; i++) {
				...
				if ((y >= clipBounds.y)
					...
					if (selectionIndex == i) {
						...
						gb.fillRect(borderLeft, y, width
								- (borderLeft + borderRight), size + space);
			...
			if (focusIndex > 0) {
				y = flFocusPosition - flSizeLUT[focusIndex - 1]
						- flSpaceLUT[focusIndex - 1];
				for (i = focusIndex - 1; i >= 0; i--) {
					if (y < borderY) {
						break;
			...
			for (i = focusIndex; i < numItems; i++) {
				...
				if (selectionIndex == i) {
					gb.setColor(hilightColor);
					/******
					 * gb.fillRect(borderLeft, y,
					 * width-(borderLeft+borderRight), size + space);
					 *******/
				}
				gb.setColor(Color.white);
				// Draw item
		...
		/*
		 * if (y1 > borderY) { gb.drawImage(up, 3*width/4, y1+5, this); } if (y2
		 * < height-borderY) { gb.drawImage(down, 3*width/4, y2-15, this); }
		 */
	...
	public void repaintDamagedItem(int index) {
		...
		repaint(borderLeft, y, getSize().width - borderLeft - borderRight,
				height);
	}
		...
		for (i = 0; i < numItems; i++) {
			if ((yStart1 == -1) && (sizeLUT[prevFocusIndex][i] > minFontSize)) {
				yStart1 = y;
					...
					&& (sizeLUT[prevFocusIndex][i] == minFontSize)) {
				height1 = y - yStart1;
				break;
			}
		...
		for (i = 0; i < numItems; i++) {
			if ((yStart2 == -1) && (sizeLUT[focusIndex][i] > minFontSize)) {
				yStart2 = y;
					...
					&& (sizeLUT[focusIndex][i] == minFontSize)) {
				height2 = y - yStart2;
				break;
			}
			y += sizeLUT[focusIndex][i] + spaceLUT[focusIndex][i];
