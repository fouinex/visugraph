...
import fr.irit.tetralogie.model.NomLongs;
import fr.irit.tetralogie.model.Sommet;
import fr.irit.tetralogie.model.metaNoeud;
...
ItemListener, MenuListener, KeyListener, Printable {
	...
	public static Logger logger = LoggerFactory.getLogger(DocumentViewer.class);
	...
	public static void main(String[] argv) {
		...
		try {
			...
			param7 = a0.substring(0, top) + "/" + param2.substring(0, 2)
					+ ".Syn";
			param_sauv = param5;
			...
			/**
			 * type : 0 -> matrice asymétrique 1 -> matrice symétrique 2 ->
			 * matrice symétrique temporelle 3 -> matrice asymétrique temporelle
			 */
			...
			for (int i = 0; i < gr.nombreSommets(); i++) {
				Sommet s = gr.getSommet(i);
				s.nomlong = (String) tgr2.nomslongs.data.get((s.nom));
			}
		...
		if (new_force == 0) {
			...
			for (int i = 0; i < graphe.nombreSommets(); i++) {
				...
				for (int j = 0; j < u.voisins.size(); j++) {
					...
					double xx = u.getX() - v.getX();
					double yy = u.getY() - v.getY();
	...
	public void MCL(String args) {
		...
		if (gr.typeMat == 1 || gr.typeMat == 2) {
			...
			System.out
			.println(" Le Clustering ne s'applique que sur des matrices symétriques");
			gr.clustering = false;
		}
			...
			if (e.getButton() == 3 && mcl) {
				...
				if (selection != null) {
					...
					if (couleur_fond == 0) {
						...
						Cluster.SetCouleursAretes(
								ConversionIntensite(control.slider3.getValue()),
								0);
					} else
						Clone.frame.setBackground(Color.black);
	...
	public void paintComponent(Graphics g) {
		...
		if (morphing != null) {
			...
			morphing.paintComponent(g, param, repmorph, couleur_fond,
					IntensitePolice, taillePolice, etape + 1);
		}
				...
				for (int i = 0; i < g.nombreSommets(); i++) {
					...
					if (!s.nom.contains("virtuel") && !s.nom.contains("Repere")) {
						...
						for (int j = 1; j <= g.instance; j++) {
							...
							nouvelCoordX += s.Metrique[j] * somVirtX[j];
							nouvelCoordY += s.Metrique[j] * somVirtY[j];
						}
